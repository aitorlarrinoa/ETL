...
replace_with_mean <- function(x){
  ...
  if(is.integer(x)){
    # reemplazo por valor constante entero
    if_else(is.na(x), as.integer(mean(x, na.rm = TRUE)), x)  # floor
  } else {
    # reemplazo por valor constante sinmas
...
how_many_na_V5 <- sum(is.na(df_simulated$V5))

media <- mean(df_simulated$V5, na.rm=TRUE)
desv <- sd(df_simulated$V5, na.rm = TRUE)

set.seed(789)
df_simulated$V5[is.na(df_simulated$V5)] <- rnorm(how_many_na, media, desv)
