...
library(readxl)
path_to_file <- "/home/rstudio/data/ejemplos_lectura.xlsx"
df_chungo <- read_xlsx(path_to_file, sheet = "Holi")
df_chungo

# ?strptime para la ayuda de tipos de %d, %D,...
# Dates as numbers
...
# El ordenador está en inglés, pero el texto está en español. Así que el ordenadro no sabe traducir
# Vamos a decirle al ordenador que trabaje en español
...
df_chungo %>% 
  select(chungo_04) %>% 
  # parse_number es más completa que as.numeric. Esta función pertenece a readr
...
#library(lubridate) es otro paquete para trabajar con fechas
#library(ISOweek) es una librería maravillosa para trabajar con datos semanales
